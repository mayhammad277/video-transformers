...
def pred_mask_for_sents(masked_list,model,tokenizer):
  ...
  for sent in masked_list:
    try :
      ...
      continue
    #lists=list(itertools.chain.from_iterable(sent_preds))
    #for lists in sent_preds:
    print("lists....................hey",sent_preds)
    #f_filled_text.write(' '.join(lists))
...
#future_masked=json.load(open('/ibex/scratch/x_hammadm/d3dhelper/MIL-NCE_HowTo100M/scripts/future_mask_GT.sjon'))
ft_mask=open('/ibex/scratch/x_hammadm/d3dhelper/MIL-NCE_HowTo100M/scripts/future_mask_sents_te.txt','r').read().splitlines()

#future_masked=[' '.join(v).split() for v in future_masked]

filled_list=pred_mask_for_sents(ft_mask,Visual_text_Model,tokenizer)
with open('future_mask_preds_te.json','w') as f:
