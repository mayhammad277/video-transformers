...
masked=open('/ibex/scratch/x_hammadm/d3dhelper/MIL-NCE_HowTo100M/scripts/masked_nn_sep_test_2M.txt','r')
masked=masked.read().splitlines()

filled_list=pred_mask_for_sents(masked,Visual_text_Model,tokenizer)
with open('mask_nn_sep_preds_test_2M_s.json','w') as f:
  json.dump(filled_list,f)

masked_vb=open('/ibex/scratch/x_hammadm/d3dhelper/MIL-NCE_HowTo100M/scripts/masked_vb_sep_test_2M.txt','r')
masked_vb=masked_vb.read().splitlines()


filled_list=pred_mask_for_sents(masked_vb,Visual_text_Model,tokenizer)
with open('mask_vb_sep_preds_test_2M_s.json','w') as f:
  json.dump(filled_list,f)
